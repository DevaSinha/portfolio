...
export interface CarouselProps {
  items?: CarouselItem[];
  baseWidth?: number; // Ignored in Embla implementation (uses CSS width), kept for compat
  autoplay?: boolean;
...
const Carousel = ({
  ...
  if (autoplay) {
    plugins.push(
      Autoplay({ delay: autoplayDelay, stopOnInteraction: true, stopOnMouseEnter: pauseOnHover })
    );
  }
        ...
        <div className="flex touch-pan-y">
          {children ? (
            ...
            items.map((item, index) => (
              ...
              >
                <div className={`relative flex flex-col p-6 h-full min-h-[400px] mx-auto
                    ${round
                ...
                >
                  <div className="text-4xl text-cyan-400 mb-4">{item.icon}</div>
